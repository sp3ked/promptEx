  ...
  if (message.action === 'insertPrompt') {
    ...
    try {
      if (hostname.includes('chatgpt.com') || hostname.includes('chat.openai.com')) {
        ...
        result = {
          success: false,
          message: 'Please navigate to a supported AI chat (ChatGPT, Claude, or Grok)'
        };
      }
    } catch (error) {
      ...
      result = {
        success: false,
        message: `Error inserting prompt: ${error.message || 'Unknown error'}`
      };
    }
...
function insertPromptChatGPT(promptText, attachments = []) {
  try {
    ...
    // Find the contenteditable div - try different possible selectors
    const possibleSelectors = [
      '#prompt-textarea', // Current primary selector
      'div.ProseMirror[contenteditable="true"]', // Old interface
      'div[data-gramm="false"][contenteditable="true"]', // Another variant
      'textarea[placeholder*="Send a message"]', // Textarea variant
    ...
    for (const selector of possibleSelectors) {
      ...
      if (element) {
        editor = element;
        console.log(`Found ChatGPT input using selector: ${selector}`);
        break;
      }
    }
    ...
    // Different insertion methods depending on element type
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'toggleSidebar') {
    const sidebar = document.getElementById('promptr-sidebar');
    if (sidebar) {
